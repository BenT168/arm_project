...
{
  size_t strgLen = strlen(str);
  char *new_string = malloc(strgLen + 1); //as there is '\0' at the end
  if(new_string == NULL){
    perror("the string is empty");
    exit(EXIT_FAILURE);
  }
...
char *push_string(char *current, const char *append)
{
  assert(current != NULL);
  assert(append != NULL);
  size_t len_concated_stg = strlen(current) + strlen(append);
  current = realloc(current, len_concated_stg + 1);
...
//memcpy(current + strlen(current), append, strlen(append)+ 1);
return current;
}
